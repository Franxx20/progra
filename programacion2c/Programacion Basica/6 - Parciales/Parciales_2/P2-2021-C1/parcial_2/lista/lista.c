        ...
        comparar comparar, acumular acumular, void *params)
{
    ...
    while(*list && comparar((*list)->data, data, params) < 0) {
        list = &(*list)->sig;
    }

    if(*list && comparar((*list)->data, data, params) == 0) {
        if(acumular != NULL) {
            int rc = acumular((*list)->data, data);
            return rc;
        }
        return DUPLICADO;
    }
        ...
        comparar comparar, acumular acumular)
{
    ...
    /* Si comparar < 0 entonces el dato es mas grande que el actual en lista */
    while(*list && comparar((*list)->data, data, NULL) != 0) {
        list = &(*list)->sig;
    }

    if(*list && comparar((*list)->data, data, NULL) == 0) {
        if(acumular != NULL) {
            int rc = acumular((*list)->data, data);
            return rc;
        }
...
void lista_ordenar(tLista *list, comparar comparar)
{
    ...
    do {
        ...
        while((*list)->sig) {
            ...
            if(comparar((*list)->data, (*list)->sig->data, NULL) > 0) {
                tLista aux = *list;
                (*list) = (*list)->sig;
                aux->sig = (*list)->sig;
                (*list)->sig = aux;
...
void lista_removerDuplicados(tLista *list, const void *data, comparar comparar)
{
    ...
    while(*list && !primer_copia) {
        if(comparar((*list)->data, data, NULL) == 0) {
            primer_copia = 1;
        }
        list = &(*list)->sig;
    }
...
size_t lista_contarDuplicados(const tLista *list, void *data, unsigned cantBytes, comparar comparar)
{
    size_t pos = 0;
...
void lista_recorrerInvertido(const tLista *list, accion accion, void *params)
{
    ...
    lista_recorrerInvertido(&aux->sig, accion, params);
    accion(aux->data, params);
}
...
tNodo **lista_buscarMenor(tLista *list, comparar comparar)
{
    ...
    return (comparar(aux->data, (*menor)->data, NULL) < 0)? list : menor;
}
...
tNodo **lista_buscarMayor(tLista *list, comparar comparar)
{
    ...
    mayor = lista_buscarMayor(&aux->sig, comparar);

    return (comparar(aux->data, (*mayor)->data, NULL) > 0)? list : mayor;
}
