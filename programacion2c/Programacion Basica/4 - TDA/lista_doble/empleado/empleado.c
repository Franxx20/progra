...
int acumularEmpleados(void *dest, const void *src);
int noAcumulados(void *dest, const void *src);
...
int leerEmpleados(const char *filename, tLista *list)
{
	...
	while (fgets(str, sizeof(str), file))
	{
		...
		if(listaLlena(list, sizeof(tEmpleado)) == 0) {
			lista_insertarOrdenado(list, &emp, sizeof(tEmpleado), compararEmpleados, noAcumulados);
		}
		else goto error;
	}
...
int filtrarEmpleados(const void *emp)
{
	...
	/*if(aux_emp->salario > 1000.0) {
		check = true;
	}*/
	if(strncmp(aux_emp->nombre, "R", 1) == 0) {
		check = true;
	}
