...
int procesarPagos_ALU(const char* pathCli, const char* pathPagos, const char* pathListado);
...
void cargarArbolRec_ALU(Arbol* pa, size_t tamElem, int li, int ls, FILE* arch, Cmp cmp);
...
int procesarPagos_ALU(const char* pathCli, const char* pathPagos, const char* pathListado)
{
    ...
    ret = cargarArbolDeArchivoOrdenado_ALU(&arbolClientesIdx, sizeof(IndCli), nombreIdx);
    if(ret != TODO_OK)
        return ret;
    ...
    while(!listaVacia_ALU(&pagos))
    {
        ...
        if(comp != 0)
        {
            ...
            if(buscarEnArbol_ALU(&arbolClientesIdx, &idx, sizeof(IndCli), comparacionArbol_ALU))
            {
                ...
                insertarEnListaOrdCDup_ALU(&deudores, &f, sizeof(FacturasImpagasFueraTerm_ALU), compararFacturasImpagas_ALU);
            }
            f.nroCliente = p.nroCliente;
            f.cantFactImpagasFueraTerm = 0;
            int comp3 = compararFecha_ALU(&p.fechaVencimiento, &p.fechaPago);
    ...
    {
        eliminarDeListaPrimero_ALU(&deudores, &deu, sizeof(FacturasImpagasFueraTerm_ALU));
        fprintf(archTxt, "%d\t%-20s\t%02d\n", deu.nroCliente, deu.nombre, deu.cantFactImpagasFueraTerm);
    }
...
int cargarListaPagos_ALU(const char* pathPagos, Lista* pl)
{
    ...
    {
        insertarEnListaOrdCDup_ALU(pl, &pago, sizeof(Pago), compararClientePago_ALU);
        fread(&pago, sizeof(Pago), 1, archPagos);
    }
...
void mostrarFacturasImp_ALU(void* dato, void* datosAccion)
{
    ...
    printf("%d\t%-20s\t%02d\n",elem->nroCliente, elem->nombre, elem->cantFactImpagasFueraTerm);
}
