...
// CASO BASE SI EL NODO ES UNA HOJA
    ...
    // CASO CONTRARIO SE COMPARA LA ALTURA DE LAS DOS ARMAS Y SE BUSCA EL REEMPLAZO
    // EN LA RAMA DE MAYOR ALTURA
    int hi  = alturaArbol(&(*pnae)->hIzq);
    int hd = alturaArbol(&(*pnae)->hDer);

    Arbol * pnareemp = hi > hd? mayorNodo(&(*pnae)->hIzq) : menorNodo(&(*pnae)->hDer);

    // COPIAMOS LOS DATOS DEL NODO A REEMPLAZO EN EL NODO A ELIMINAR
    (*pnae)->dato  = (*pnareemp)->dato;
    (*pnae)->tamDato = (*pnae)->tamDato;
