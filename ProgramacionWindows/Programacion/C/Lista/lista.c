...
int buscarEnListaOrd(const Lista *l, void *dato, unsigned tamDato, Cmp cmp)
{
    ...
    // se avanza mientras haya nodos y el dato de la lista lista sea menor al dato buscado
    ...
    // si ya no hay mas nodos o el dato de la lista es mayor al buscado lo que significa que el dato buscado no existe en la lista
...
{
    int pos = 0;
    // se avanza el puntero mientras haya nodoso y el dato de la lista sea distinto al dato buscado
    while (*l && cmp((*l)->dato, dato) != 0)
    {
        l = &(*l)->sig;
        pos++;
...
bool verEnPosicionDeLista(const Lista *l, void *dato, int pos)
{
    ...
    // si la lista no tiene mas nodos o el dato no existe
    if (*l == NULL || pos < 0)
        return pos = -1;

    memcpy(dato, (*l)->dato, MIN(dato, (*l)->dato));
