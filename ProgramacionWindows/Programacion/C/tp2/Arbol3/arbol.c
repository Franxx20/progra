...
bool cargarEnArbolDeUnArchivoOrdenado(Arbol *t, int li, int ls, int tamDato,
                                      FILE *fp, Cmp cmp,
                                      Actualizar actualizar) {
  if (li > ls)
    return false;
  ...
  cargarEnArbolDeUnArchivoOrdenado(t, li, mitad - 1, tamDato, fp, cmp,
                                   actualizar);
  cargarEnArbolDeUnArchivoOrdenado(t, mitad + 1, ls, tamDato, fp, cmp,
                                   actualizar);
...
int podarArbol(Arbol *t, int nivel) {
  ...
  return podarArbol(&(*t)->hIzq, nivel - 1) +
         podarArbol(&(*t)->hDer, nivel - 1);
}
...
int contarNodoArbolHastaNivel(const Arbol *t, int nivel) {
  ...
  return contarNodoArbolHastaNivel(&(*t)->hIzq, nivel - 1) +
         contarNodoArbolHastaNivel(&(*t)->hDer, nivel - 1) + 1;
}
